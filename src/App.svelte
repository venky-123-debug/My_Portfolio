...
onMount(() => {
  ...
  // Start using the canvas context (e.g., drawing, updating, etc.)
});
...
game.keydown = function (e) {
  // If the left arrow key or 'A' key is pressed, move the player left.
  if (e.key === "ArrowLeft" || e.key === "a") {
    game.player.update(-5, 0);
  }
  // If the right arrow key or 'D' key is pressed, move the player right.
...
<div class="text-3xl flex justify-center mt-10 mx-auto text-blue-500 font-semibold">Space Invaders</div>
<div class="bg-blue-50 flex items-center justify-center overflow-hidden">
  <canvas bind:this={canvas} id="canvas" class="w-[800px] h-[600px]" />
</div>
